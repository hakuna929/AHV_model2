...
%   其中 CT 为推力系数（用于 T = CT * qbar * S_ref）
...
while k <= N_max && t_now <= T_end
    ...
    % aero_coeffs 同时给出 CL/CD/CT（CT 为推力系数）
    [CL, CD, CT_now] = aero_coeffs(Ma, alpha_cmd, dT);

    L = CL*qbar*S_ref;
    D = CD*qbar*S_ref;

    % 仍用阻力前馈（更稳），速度环输出 CT_cmd，再反解 dT
    ...
    % CT->dT 反解：改为使用 aero_coeffs 的 CT(dT) 而不是 thrust_coeffs
    ...
    % 升力方向：沿"指令加速度的法向分量"方向（更符合3DOF a_cmd 框架）
    % 先把 a_cmd 去掉沿速度方向的分量，得到"法向加速度"方向
    a_n = a_cmd_ecef - dot(a_cmd_ecef, vhat) * vhat;
    a_n_norm = norm(a_n);

    if a_n_norm > 1e-8
        lift_dir = a_n / a_n_norm;
    else
        % 退化情况：用当地竖直向上
        lift_dir = u_up;
    end
